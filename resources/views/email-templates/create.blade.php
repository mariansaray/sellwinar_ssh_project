...
@section('title', 'Nová e-mail šablóna — Sellwinar')
...
@section('content')
    <div class="max-w-2xl">
        <h1 class="font-heading text-3xl font-bold text-ink-800 dark:text-white mb-6">Nová e-mail šablóna</h1>
        ...
        <form method="POST" action="{{ route('dashboard.email-templates.store') }}" class="space-y-6">
            @csrf

            <div>
                <label class="block text-sm font-medium text-ink-800 dark:text-ink-100 mb-1.5">Názov šablóny</label>
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="Napr. Potvrdenie registrácie — profesionálny"
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="delay_minutes" value="{{ old('delay_minutes', 0) }}" required
                           class="w-full px-3.5 py-2.5 bg-white dark:bg-ink-800 border border-ink-200 dark:border-ink-600 rounded-md text-sm focus:border-violet-600 ring-violet-focus transition-fast">
                    <p class="text-xs text-ink-400 mt-1">Záporné = pred webinárom (napr. -60 = 1h pred). Kladné = po (napr. 240 = 4h po).</p>
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-ink-800 dark:text-ink-100 mb-1.5">Predmet e-mailu</label>
                <input type="text" name="subject" value="{{ old('subject') }}" required placeholder="Potvrdenie registrácie — @{{nazov_webinara}}"
                       class="w-full px-3.5 py-2.5 bg-white dark:bg-ink-800 border border-ink-200 dark:border-ink-600 rounded-md text-sm focus:border-violet-600 ring-violet-focus transition-fast">
            </div>
            ...
            <div>
                ...
                <textarea name="body_html" rows="12" required
                          ...
                          placeholder="<p>Dobrý deň @{{meno}},</p><p>Ďakujeme za registráciu...</p>">{{ old('body_html') }}</textarea>
                <p class="text-xs text-ink-400 mt-1">Placeholdery: @{{meno}}, @{{email}}, @{{nazov_webinara}}, @{{datum_webinara}}, @{{cas_webinara}}, @{{link_na_webinar}}</p>
            </div>
